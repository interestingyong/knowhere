...
class OdinANNMeta {
 ...
 private:
    OdinANNBuildConfig build_params_;
    int64_t num_elem_;
...
struct OdinANNQueryConfig {
    uint64_t k;
    uint32_t search_list_size;
    uint32_t beamwidth;
    uint32_t vectors_beamwidth;

    NLOHMANN_DEFINE_TYPE_INTRUSIVE(OdinANNQueryConfig, k, search_list_size, beamwidth, vectors_beamwidth)
};
...
class OdinANNVisitInfo {
 public:
    OdinANNVisitInfo() = default;
    ...
    NLOHMANN_DEFINE_TYPE_INTRUSIVE(OdinANNVisitInfo, query_params_, infos_)

 private:
    OdinANNQueryConfig query_params_;
    std::vector<TopCandidateInfo> infos_;
};

struct FederResult {
    OdinANNVisitInfo visit_info_;
